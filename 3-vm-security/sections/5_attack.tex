...
With the problem formulated as a search problem, we now present our search strategy. We decide to design the search as a genetic algorithm~\cite{whitley1994genetic}. The reasons for this choice are as follow:
...
The results of running Parity on a more powerful bare-metal server show that even such machines are relatively vulnerable to our attack. Indeed, Parity, which was the fastest of the tree clients, still took more than~\empirical{18} seconds to execute the transactions. An important point to notice is that the IO throughput is considerably higher on our bare-metal server, which is most likely one of the main reason for the speedup.
...
% One of the key value proposition of Ethereum, and the base of its security model, is \emph{decentralisation}~of full nodes (and miners but this is out of scope)~\cite{Buterin2014,DBLP:journals/corr/abs-1801-03998}. The current requirements to run a full node on the Ethereum main net are low enough for most commodity hardware to be able to keep up without issue. The only point mentioned by the Ethereum developers is that running a full node requires an SSD~\cite{ethereum-faq}. Although there is currently no official documentation on other requirements, other sources estimate the minimum required memory to be about 8GB~\cite{node-incentive,pantheon-system-requirements,eth-hardware-requirements}.
...
% To measure the potential effect of the attack on the Ethereum network, we fork the main Ethereum network at block~\empirical{5,171,602} and create a small network composed of~\empirical{3} full nodes. As the difficulty of the network as this point is clearly too high to be able to mine, we skip the validation and force the nodes to generate a new block roughly every~13 seconds, to mimic the behavior of the main Ethereum network.

%\subsection{Finding Resource Exhaustion Attacks in the Wild}

% \begin{figure}[tb]
%     \centering
%     \begin{tabular}{lll}
%         \toprule
%          \bf Address &  \bf Throughput & \bf Explanation \\
%          & \\
%          \bottomrule
%     \end{tabular}
%     % \includegraphics{}
%     \caption{Twenty lowest-throughput contracts found in the wild.}
%     \label{fig:}
% \end{figure}
